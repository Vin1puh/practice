...
export default function MainPage() {
    ...
    return (
        <main className=''>
            ...
            <section className='w-7/10 flex justify-between items-start'>
                <div className='flex flex-col items-end justify-between h-[700px] w-65/100'>
                    ...
                    <form className='flex bg-white w-full justify-center items-center h-[500px] rounded-[1rem]' action="">
                    ...
                    <NavLink to='/search' className='w-[300px] h-[60px] bg-[#009661] rounded-[1rem] text-white text-[2rem] flex items-center justify-center duration-330 hover:scale-[1.05]'>
                        Поиск ({results} Результатов)
                    </NavLink>
                </div>
                {Card.slice(0, 1).map((item, index) => (
                    <NavLink key={index} to='/car_page' state={{carData: item}} className='relative w-3/10 h-[600px] bg-white rounded-[1rem] flex justify-center items-center'>
                        <div className='w-9/10 h-95/100 flex flex-col justify-between items-center'>
                            <img src={Images.daily} alt="" className='absolute left-57/100 top-7/100' />
                            <img src={item.image} alt="" className='rounded-[1rem] w-full max-h-55/100' />
                            <h2 className='text-[2.2rem]'>{item.mark} {item.model}</h2>
                            <p className='text-[2rem] text-gray-500 text-start w-full'>{item.stat}</p>
                            <div className='w-full h-[70px] flex items-center justify-around bg-[#F9F9F9] rounded-[1rem]'>
            ...
            <section className='w-7/10 flex justify-between items-start flex-col'>
                ...
                <div className='w-full flex justify-center items-center h-[60px]'>
                    <button onClick={handleClick} className='w-[222px] h-full bg-[#009661] text-white text-[2rem] rounded-[1rem]'>Больше объявлений</button>
                </div>
            </section>
            <div className='h-[100px]'></div>
            <section className='w-95/100 h-[550px] flex justify-center items-center bg-white'>
                <div className='w-737/1000 h-1/2 flex justify-between items-center'>
                    <div className='w-4/10 h-full flex items-start justify-between flex-col'>
                        <h1 className='text-[4rem]'>Наши продавцы</h1>
                        <p className='text-[2rem] text-gray-500'>Мы работаем только  с лучшимы игроками на рынке. Безопасность сделки обеспечена</p>
                        <button className='bg-[#009661] text-white w-[200px] h-[60px] text-[2rem] rounded-[1rem]'>показать всех</button>
                    </div>
